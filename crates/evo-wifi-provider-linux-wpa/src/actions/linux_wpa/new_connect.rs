use evo_wifi_core::borrowed::{
    ...
    WifiNetworkSelectionInputBorrowed, WifiPasswordInputBorrowed,
};
use evo_wifi_core::contracts::WifiNewNetworkConnectContract;
use std::process::{Command, Output};
use std::thread;
use std::time::Duration;

#[derive(Clone, Copy, Debug, Default)]
pub struct LinuxWpaWifiNewNetworkConnectProvider;

impl WifiNewNetworkConnectContract for LinuxWpaWifiNewNetworkConnectProvider {
    fn provide<R>(
        &self,
        ...
        if configure_network(interface.name, network_id, ssid, password).is_none() {
            let _ = remove_network(interface.name, network_id);
            return None;
        }

        if connect_network(interface.name, network_id).is_none() {
            let _ = remove_network(interface.name, network_id);
            return None;
        }
        ...
        if !status.state.is_completed() || status.ssid != ssid {
            let _ = remove_network(interface.name, network_id);
            return None;
        }
...
fn add_network(interface_name: &str) -> Option<u32> {
...
fn connect_network(interface_name: &str, network_id: u32) -> Option<()> {
    let network_id = network_id.to_string();
...
fn remove_network(interface_name: &str, network_id: u32) -> Option<()> {
    ensure_wpa_cli_ok(
        Command::new("wpa_cli")
            .arg("-i")
            .arg(interface_name)
